...
def create_gui():
    ...
    tk.Label(root, text="源目录:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
    ...
    tk.Label(root, text="最大日志行数:").grid(row=4, column=0, sticky=tk.W, padx=5, pady=5)
    ...
    tk.Label(root, text="最大备份数:").grid(row=5, column=0, sticky=tk.W, padx=5, pady=5)
    ...
    tk.Label(root, text="游戏可执行文件:").grid(row=6, column=0, sticky=tk.W, padx=5, pady=5)
    ...
    tk.Label(root, text="日志文件名:").grid(row=7, column=0, sticky=tk.W, padx=5, pady=5)
    ...
    tk.Label(root, text="文件匹配模式（每行一个）:").grid(row=8, column=0, sticky=tk.W, padx=5, pady=5)
    ...
    def save_and_exit():
        try:
            ...
            messagebox.showerror("输入错误", "请确保数字字段为有效整数！")
            return
        save_config(new_config)
        messagebox.showinfo("成功", "配置已保存！")
        root.destroy()

    tk.Button(root, text="保存配置", command=save_and_exit).grid(row=9, column=1, pady=10)
    root.mainloop()
